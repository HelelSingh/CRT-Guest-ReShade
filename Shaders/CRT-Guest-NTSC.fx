...
float4 PreShaderPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	if(int(TNTC)==1)
	{
	color1=texCD(NTSC_L01,coord1);
	color2=texCD(NTSC_L01,coord2);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==2)
	{
	color1=texCD(NTSC_L02,coord1);
	color2=texCD(NTSC_L02,coord2);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==3)
	{
	color1=texCD(NTSC_L03,coord1);
	color2=texCD(NTSC_L03,coord2);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==4)
	{
	color1=texCD(NTSC_L04,coord1);
	color2=texCD(NTSC_L04,coord2);
	res=lerp(color1,color2,f);
	}
