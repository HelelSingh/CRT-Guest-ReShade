/*
	...
	CRT - Guest - Advanced (Copyright (C) 2018-2024 guest(r) - guest.r@gmail.com)
...
#define GUEST_S0 ReShade::BackBuffer

texture GUEST_T1{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA32F;};
sampler GUEST_S1{Texture=GUEST_T1;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=POINT ;MinFilter=POINT ;MipFilter=POINT ;};

texture GUEST_T2{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA16F;};
sampler GUEST_S2{Texture=GUEST_T2;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=POINT ;MinFilter=POINT ;MipFilter=POINT ;};

texture GUEST_T3{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA16F;};
sampler GUEST_S3{Texture=GUEST_T3;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T4{Width=Resolution_X;Height=Resolution_Y ;Format=RGBA16F;};
sampler GUEST_S4{Texture=GUEST_T4;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T5{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
sampler GUEST_S5{Texture=GUEST_T5;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T6{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
sampler GUEST_S6{Texture=GUEST_T6;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T7{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
sampler GUEST_S7{Texture=GUEST_T7;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T8{Width=800.00000000;Height=600.00000000 ;Format=RGBA16F;};
sampler GUEST_S8{Texture=GUEST_T8;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_T9{Width=BUFFER_WIDTH;Height=BUFFER_HEIGHT;Format=RGBA16F;};
sampler GUEST_S9{Texture=GUEST_T9;AddressU=BORDER;AddressV=BORDER;AddressW=BORDER;MagFilter=LINEAR;MinFilter=LINEAR;MipFilter=LINEAR;};

texture GUEST_01<source="CRT-LUT-1.png";>{Width=1024;Height=32;};
sampler GUEST_L1{Texture=GUEST_01;};

texture GUEST_02<source="CRT-LUT-2.png";>{Width=1024;Height=32;};
sampler GUEST_L2{Texture=GUEST_02;};

texture GUEST_03<source="CRT-LUT-3.png";>{Width=1024;Height=32;};
sampler GUEST_L3{Texture=GUEST_03;};

texture GUEST_04<source="CRT-LUT-4.png";>{Width=1024;Height=32;};
sampler GUEST_L4{Texture=GUEST_04;};
...
float3 fetch_pixel(float2 coord)
{
	...
	float2 d2=dx-dy;
	float sum=15.0;
	float3 result=3.0*COMPAT_TEXTURE(GUEST_S2,coord).rgb+2.0*COMPAT_TEXTURE(GUEST_S2,coord+dx).rgb+2.0*COMPAT_TEXTURE(GUEST_S2,coord-dx).rgb+
	2.0*COMPAT_TEXTURE(GUEST_S2,coord+dy).rgb+2.0*COMPAT_TEXTURE(GUEST_S2,coord-dy).rgb+COMPAT_TEXTURE(GUEST_S2,coord+d1).rgb+
	COMPAT_TEXTURE(GUEST_S2,coord-d1).rgb+COMPAT_TEXTURE(GUEST_S2,coord+d2).rgb+COMPAT_TEXTURE(GUEST_S2,coord-d2).rgb;
	return result/sum;
}
...
void bring_pixel(inout float3 c,inout float3 b,inout float3 g,float2 coord,float2 boord)
{
	...
	float r1=COMPAT_TEXTURE(GUEST_S9,coord+rc).r;
	float g1=COMPAT_TEXTURE(GUEST_S9,coord+gc).g;
	float b1=COMPAT_TEXTURE(GUEST_S9,coord+bc).b;
	float ds=decons;
	float3 d=float3(r1,g1,b1);
	c=clamp(lerp(c,d,ds),0.0,1.0);
	r1=COMPAT_TEXTURE(GUEST_S8,boord+rc).r;
	g1=COMPAT_TEXTURE(GUEST_S8,boord+gc).g;
	b1=COMPAT_TEXTURE(GUEST_S8,boord+bc).b;
	d=float3(r1,g1,b1);
	b=g=lerp(b,d,min(ds,1.0));
	r1=COMPAT_TEXTURE(GUEST_S6,boord+rc).r;
	g1=COMPAT_TEXTURE(GUEST_S6,boord+gc).g;
	b1=COMPAT_TEXTURE(GUEST_S6,boord+bc).b;
	d=float3(r1,g1,b1);
...
float4 AfterglowPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float w=1.0;
	float3 color0=COMPAT_TEXTURE(GUEST_S0,texcoord.xy   ).rgb;
	float3 color1=COMPAT_TEXTURE(GUEST_S0,texcoord.xy-dx).rgb;
	float3 color2=COMPAT_TEXTURE(GUEST_S0,texcoord.xy+dx).rgb;
	float3 color3=COMPAT_TEXTURE(GUEST_S0,texcoord.xy-dy).rgb;
	float3 color4=COMPAT_TEXTURE(GUEST_S0,texcoord.xy+dy).rgb;
	float3 clr=(2.5*color0+color1+color2+color3+color4)/6.5;
	float3 a=COMPAT_TEXTURE(GUEST_S1,texcoord.xy).rgb;
...
float4 PreShaderPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float4 imgColor=COMPAT_TEXTURE(GUEST_S0,texcoord.xy);
	float4 aftrglow=COMPAT_TEXTURE(GUEST_S1,texcoord.xy);
	...
	if(int(TNTC)==1)
	{
	color1=COMPAT_TEXTURE(GUEST_L1,coord0);
	color2=COMPAT_TEXTURE(GUEST_L1,coord1);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==2)
	{
	color1=COMPAT_TEXTURE(GUEST_L2,coord0);
	color2=COMPAT_TEXTURE(GUEST_L2,coord1);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==3)
	{
	color1=COMPAT_TEXTURE(GUEST_L3,coord0);
	color2=COMPAT_TEXTURE(GUEST_L3,coord1);
	res=lerp(color1,color2,f);
	}else
	if(int(TNTC)==4)
	{
	color1=COMPAT_TEXTURE(GUEST_L4,coord0);
	color2=COMPAT_TEXTURE(GUEST_L4,coord1);
	res=lerp(color1,color2,f);
	}
...
float4 LuminancePS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float2 y2=2.0*dy;
	float ltotal=0.0;
	ltotal+=length( tex2Dlod(GUEST_S2,float4(float2(0.3,0.3),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S2,float4(float2(0.3,0.7),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S2,float4(float2(0.7,0.3),m,0)).rgb);
	ltotal+=length( tex2Dlod(GUEST_S2,float4(float2(0.7,0.7),m,0)).rgb);
	ltotal*=0.25;
	ltotal=pow(0.577350269*ltotal,0.7);
	float lhistory=tex2D(GUEST_S3,0.5).a;
	ltotal=lerp(ltotal,lhistory,lsmooth);
	float3 l1=COMPAT_TEXTURE(GUEST_S2,fuxcoord.xy   ).rgb;
	float3 r1=COMPAT_TEXTURE(GUEST_S2,fuxcoord.xy+dx).rgb;
	float3 l2=COMPAT_TEXTURE(GUEST_S2,fuxcoord.xy-dx).rgb;
	float3 r2=COMPAT_TEXTURE(GUEST_S2,fuxcoord.xy+x2).rgb;
	float c1=dist(l2,l1);
	float c2=dist(l1,r1);
	float c3=dist(r2,r1);
...
{
	float3 c1=COMPAT_TEXTURE(GUEST_S2,fuxcoord).rgb;
	float3 c2=COMPAT_TEXTURE(GUEST_S2,fuxcoord+float2(0.0,OrgSize.w)).rgb;
	...
	if(inter <=OrgSize.y/yres_div&&interm>0.5&&intres!=1.0&&intres!=0.5&&vga_mode<0.5||hscan)
	{
	intera=0.25;
	...
	if(vga_mode>0.5)
	{
	c=c1;if(inter<=OrgSize.y)intera=0.75; else intera=0.5;
	}
	c=pow(c,gamma_in);
...
float4 HGaussianPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=COMPAT_TEXTURE(GUEST_S4,tex+n*dx).rgb;
	if(m_glow>0.5)
	{
...
float4 VGaussianPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=COMPAT_TEXTURE(GUEST_S5,tex+n*dy).rgb;
	w=gauss_v(n+f);
	color=color+w*pixel;
	wsum=wsum+w;
	n=n+1.0;
...
float4 BloomHorzPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=COMPAT_TEXTURE(GUEST_S4,tex+n*dx);
	w=bloom_h(n+f);
...
float4 BloomVertPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	do
	{
	pixel=COMPAT_TEXTURE(GUEST_S7,tex+n*dy);
	w=bloom_v(n+f);
	pixel.a*=pixel.a*pixel.a;
	color=color+w*pixel;
	wsum=wsum+w;
...
{
	float2 prescalex=(tex2Dsize(GUEST_S4,0))/OrgSize.xy;
	float4 AdvSize=OrgSize*lerp(float4(prescalex.x,1.0,1.0/prescalex.x,1.0),float4(1.0,prescalex.y,1.0,1.0/prescalex.y),TATE);
	float gamma_in=1.0/COMPAT_TEXTURE(GUEST_S4,0.25).a;
	float lum=COMPAT_TEXTURE(GUEST_S3,0.5).a;
	float intera=COMPAT_TEXTURE(GUEST_S4,float2(0.75,0.25)).a;
	bool hscan=(hiscan>0.5);
	...
	if((intres==0.5&& notate&&prescalex.y<1.5)|| vgascan          )pC4.y=floor(pC4.y*OrgSize.y)*OrgSize.w+0.5*OrgSize.w;
	if((intres==0.5&&!notate&&prescalex.x<1.5)||(vgascan&&!notate))pC4.x=floor(pC4.x*OrgSize.x)*OrgSize.z+0.5*OrgSize.z;
	...
	if(hscan)pC4=lerp(float2(pC4.x,pos.y),float2(pos.x,pC4.y),TATE);
	float zero=exp2(-h_sharp);
	float idiv=clamp(lerp(AdvSize.x,AdvSize.y,TATE)/400.0,1.0,2.0);
	float fdivider=max(min(lerp(prescalex.x,prescalex.y,TATE),2.0),idiv*float(interb));
	...
	{
	twl3=0.0;twr3=0.0;
	c1=COMPAT_TEXTURE(GUEST_S3,pC4     ).xyz;
	c2=COMPAT_TEXTURE(GUEST_S3,pC4+offy).xyz;
	c1=max(c1-sth,0.0);
	c2=max(c2-sth,0.0);
	}
	float3 l3,l2,l1,r1,r2,r3,color1,color2,colmin,colmax;
	l3=COMPAT_TEXTURE(GUEST_S4,pC4-off2     ).rgb;
	l2=COMPAT_TEXTURE(GUEST_S4,pC4-offx     ).rgb;
	l1=COMPAT_TEXTURE(GUEST_S4,pC4          ).rgb;
	r1=COMPAT_TEXTURE(GUEST_S4,pC4+offx     ).rgb;
	r2=COMPAT_TEXTURE(GUEST_S4,pC4+off2     ).rgb;
	r3=COMPAT_TEXTURE(GUEST_S4,pC4+offx+off2).rgb;
	...
	float3 scolor1=clamp(lerp(max(max(color1.r,color1.g),color1.b),fscolor1,spike),0.0,1.0);
	if(!interb)color1=pow(color1,scangamma/gamma_in);
	float3 scolor2;
	if(!interb&&!hscan)
	{
	pC4+=offy;
	if((intres==0.5&& notate&&prescalex.y<1.5)|| vgascan          )pC4.y=floor((pos.y+0.33*offy.y)*OrgSize.y)*OrgSize.w+0.5*OrgSize.w;
	if((intres==0.5&&!notate&&prescalex.x<1.5)||(vgascan&&!notate))pC4.x=floor((pos.x+0.33*offy.x)*OrgSize.x)*OrgSize.z+0.5*OrgSize.z;
	l3=COMPAT_TEXTURE(GUEST_S4,pC4-off2     ).rgb;
	l2=COMPAT_TEXTURE(GUEST_S4,pC4-offx     ).rgb;
	l1=COMPAT_TEXTURE(GUEST_S4,pC4          ).rgb;
	r1=COMPAT_TEXTURE(GUEST_S4,pC4+offx     ).rgb;
	...
	scolor2=clamp(lerp(max(max(color2.r,color2.g),color2.b),fscolor2,spike),0.0,1.0);
...
float4 ChromaticPS(float4 position:SV_Position,float2 texcoord:TEXCOORD):SV_Target
{
	...
	float2 pos=warp(lexcoord);
	float3 color=COMPAT_TEXTURE(GUEST_S9,pos1).rgb;
	float3 Bloom=COMPAT_TEXTURE(GUEST_S8,pos).rgb;
	float3 Glow=COMPAT_TEXTURE(GUEST_S6,pos).rgb;
	...
	float mx1=COMPAT_TEXTURE(GUEST_S9,pos1   ).a;
	...
	float mx0=COMPAT_TEXTURE(GUEST_S9,pos1-dx).a;
	float mx2=COMPAT_TEXTURE(GUEST_S9,pos1+dx).a;
	...
	mx0=COMPAT_TEXTURE(GUEST_S9,pos1-dx).a;
	mx2=COMPAT_TEXTURE(GUEST_S9,pos1+dx).a;
	...
	if(shadow_msk> 0.5)
	{
	float2 maskcoord=fracoord.yx* 1.00001;
	...
	float3 Ref=COMPAT_TEXTURE(GUEST_S4,pos).rgb;
	float maxb=COMPAT_TEXTURE(GUEST_S8,pos).a;
...
technique CRT_Guest_Advanced
{
	...
	RenderTarget=GUEST_T1;
	}
	pass PreShader
	{
	VertexShader=PostProcessVS;
	PixelShader=PreShaderPS;
	RenderTarget=GUEST_T2;
	}
	pass Luminance
	{
	VertexShader=PostProcessVS;
	PixelShader=LuminancePS;
	RenderTarget=GUEST_T3;
	}
	pass Linearize
	{
	VertexShader=PostProcessVS;
	PixelShader=LinearizePS;
	RenderTarget=GUEST_T4;
	}
	pass GaussianX
	{
	VertexShader=PostProcessVS;
	PixelShader=HGaussianPS;
	RenderTarget=GUEST_T5;
	}
	pass GaussianY
	{
	VertexShader=PostProcessVS;
	PixelShader=VGaussianPS;
	RenderTarget=GUEST_T6;
	}
	pass BloomHorz
	{
	VertexShader=PostProcessVS;
	PixelShader=BloomHorzPS;
	RenderTarget=GUEST_T7;
	}
	pass BloomVert
	{
	VertexShader=PostProcessVS;
	PixelShader=BloomVertPS;
	RenderTarget=GUEST_T8;
	}
	pass Guest_CRT
	{
	VertexShader=PostProcessVS;
	PixelShader=Advanced_PS;
	RenderTarget=GUEST_T9;
	}
	pass Chromatic
	{
	VertexShader=PostProcessVS;
